...
public class CategoriaController {
	 ...
	 * mostrarIndex(Model model)`: Define el método. Aunque aquí se recibe un objeto `Model`, en este ejemplo no se usa para pasar datos a la vista, pero es una buena
	 * práctica incluirlo si se planea usarlo en el futuro. `return "categorias/listCategorias"`: El método devuelve la cadena `"categorias/listCategorias"`. Spring
	 * interpretará esto como el nombre lógico de la vista. Buscará una plantilla HTML (por ejemplo, `listCategorias.html`) dentro de la carpeta `templates/categorias/` y la
	 * renderizará para el usuario. ¿Por qué se implementa así? - **Mapeo de URL**: Es la forma estándar de mapear una URL específica a un método controlador en Spring MVC. -
