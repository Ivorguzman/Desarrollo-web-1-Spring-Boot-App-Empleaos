 ...
 * ¿Qué es esta clase? Es el Controlador Principal de la aplicación. Actúa como el recepcionista para las URLs más comunes, como la página de inicio.
 *
 * ¿Cómo funciona?
 ...
 *   A diferencia de `@RestController`, esta clase está diseñada para devolver nombres de vistas (HTML) que serán renderizadas, no solo datos crudos (como JSON).
 *
 * ¿Por qué se implementa así?
 * - **Punto de Entrada**: Sirve como el punto de entrada principal para los usuarios que visitan el sitio.
 * - **Patrón MVC**: Sigue el patrón Modelo-Vista-Controlador, separando la lógica de la presentación y los datos, lo que hace la aplicación más organizada y mantenible.
 */
@Controller
public class HomeController {
	 ...
	 * - `@Autowired`: Esta es una anotación clave de Spring. Le dice al framework: "Busca un bean (un objeto gestionado por ti) que sea del tipo `Itf_VacanteService`
	 ...
	 *
	 * ¿Cómo lo logra?
	 * - `@GetMapping("/")`: Es una anotación que mapea este método exclusivamente a peticiones HTTP GET para la URL raíz.
	 * - `List<Vacante> listaDeVacantes = serviceVacantes.buscarTodas();`: Llama al servicio para obtener la lista completa de vacantes disponibles.
	 * - `if (!listaDeVacantes.isEmpty())`: Realiza una comprobación de seguridad para asegurarse de que la lista no esté vacía antes de intentar usarla.
	 * - `modelo.addAttribute("vacantes", listaDeVacantes)`: Añade la lista completa de vacantes al "modelo". El modelo es un objeto que actúa como un puente para pasar datos desde el controlador hasta la vista (el archivo HTML).
	 * - `return "home"`: Devuelve el nombre lógico de la vista. Spring buscará un archivo llamado `home.html` en la carpeta `templates/` y se lo enviará al navegador del usuario, dándole acceso a los datos que se añadieron al modelo.
	 *
	 * ¿Por qué se implementa así?
	 * - **Página de Inicio Dinámica**: En lugar de una página estática, la página de inicio muestra datos reales y actualizados (las vacantes) cada vez que se carga.
	 * - **Lógica en el Controlador**: La responsabilidad de obtener los datos y pasarlos a la vista recae en el controlador, manteniendo la vista (HTML) lo más simple posible.
	 */
	@GetMapping("/")
	 ...
	 *
	 * ¿Cómo lo logra?
	 * - `@GetMapping("/tabla")`: Mapea este método a la URL `/tabla`.
	 * - `List<Vacante> listaDeVacantes = serviceVacantes.buscarTodas();`: Al igual que en `mostrarHome`, obtiene la lista completa de vacantes del servicio.
	 * - `modelo.addAttribute("vacantes", listaDeVacantes)`: Pasa la lista completa al modelo para que la vista `tabla.html` pueda acceder a ella.
	 * - `return "tabla"`: Le dice a Spring que renderice la vista `tabla.html`, la cual se espera que contenga la lógica (por ejemplo, un bucle `th:each` de Thymeleaf) para iterar sobre la lista de vacantes y construir una tabla HTML.
	 *
	 * ¿Por qué se implementa así?
	 * - **Vista Especializada**: Proporciona una vista dedicada a mostrar los datos de una manera específica (una tabla), separada de la página de inicio.
	 * - **Reutilización de Lógica**: Nótese que la lógica para obtener los datos es la misma que en `mostrarHome`. El controlador actúa como un director que decide qué datos enviar y a qué vista.
	 */
	@GetMapping("/tabla")
