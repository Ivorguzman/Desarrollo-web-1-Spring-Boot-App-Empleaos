/**
 * La declaración de paquete define el espacio de nombres donde reside esta clase. Ayuda a organizar el código y
 * a prevenir conflictos de nombres. En este caso, la clase HomeController pertenece al paquete
 * 'net.itinajero.controller'.
 */
...
/**
 * Controlador principal de la aplicación que maneja las peticiones web para la página de inicio y otras páginas
 * generales. La anotación @Controller le indica al contenedor de Spring que esta clase es un componente de tipo
 * "Controlador" en el patrón MVC. Spring la escaneará, creará una instancia (un "bean") y la gestionará en su
 * ciclo de vida.
 *
 * @author Ivor
 */
...
/**
 * Creación de una instancia del logger para esta clase. 'private static final': Es una constante (final)
 * compartida (static) por todas las instancias de HomeController. El logger se usa para registrar mensajes
 * sobre el flujo de la aplicación, lo cual es una práctica estándar para depuración y monitoreo.
 */
 ...
 *
 * @GetMapping("/"): Mapea este método a la URL raíz. Cada vez que un usuario visite "http://localhost:8080/",
...
/**
 * Método que maneja las peticiones HTTP GET a la URL "/tabla". Su propósito es obtener la lista de todas las
 * vacantes desde la capa de servicio y mostrarla en una vista.
 *
 * @param model
...
/**
 * Método que maneja las peticiones HTTP GET a la URL "/listado". Muestra un ejemplo de cómo pasar una lista
 * simple de Strings a una vista.
 *
 * @param modelo
...
public String mostrarListado(Model modelo) {
	...
	List<String> lista = List.of("Ingeniería de Software", "Auxiliar Contable", "Vendedor",
...
/**
 * Método que maneja las peticiones HTTP GET a la URL "/detalle". Muestra un ejemplo de cómo pasar un único
 * objeto complejo (una Vacante) a una vista.
 *
 * @param modelo
...
public String mostrarDetalles(Model modelo) {
	...
	vacante.setDescripcion(
			"Vacante para ingeniero de comunicaciones con experiencia en redes y telecomunicaciones.");
