...
/**
 * ¿Qué es esta clase? Esta clase, `Vacante`, es un "Modelo" o "POJO" (Plain Old Java Object). Actúa como un
 * plano o plantilla para crear objetos que representarán una oferta de trabajo en nuestra aplicación. Contiene
 * los datos (atributos) y el comportamiento básico (métodos) de una vacante. ¿Cómo funciona? Define las
 * características que tendrá cada vacante (como ID, nombre, descripción, etc.) y proporciona métodos para
 * acceder y modificar esos datos. ¿Por qué se implementa así? - **Representación de Datos**: Es la forma
 * estándar de representar entidades de negocio en Java. Cada objeto `Vacante` contendrá la información de una
 * única oferta de trabajo. - **Encapsulación**: Al mantener los atributos privados y exponerlos a través de
 * métodos públicos (getters y setters), se controla cómo se accede y modifica la información, lo que mejora la
 * seguridad y el mantenimiento del código. - **Reutilización**: Este modelo puede ser utilizado por diferentes
 * partes de la aplicación (controladores, servicios, repositorios) para manejar los datos de las vacantes.
 */
...
/**
 * ¿Qué son estas constantes? Son valores numéricos fijos que definen los umbrales para clasificar el salario de
 * una vacante en diferentes niveles (bajo, normal, alto). ¿Cómo funcionan? Se declaran como `private static
 * final`: - `private`: Solo accesibles dentro de esta clase. - `static`: Pertenecen a la clase, no a una
 * instancia específica. Solo hay una copia de estas variables, compartida por todos los objetos `Vacante`. -
 * `final`: Su valor no puede ser cambiado una vez inicializado. Son inmutables. ¿Por qué se usan? - **Evitar
 * "números mágicos"**: En lugar de usar `2000.0` directamente en el código, usamos un nombre descriptivo
 * (`UMBRAL_SALARIO_BAJO`), lo que mejora la legibilidad. - **Facilidad de Mantenimiento**: Si los umbrales
 * cambian en el futuro, solo necesitas modificar el valor en un único lugar (aquí), en vez  de buscar y cambiar
 * cada ocurrencia en el código. - **Claridad**: Hacen que el propósito de los valores sea evidente.
 */
...
/**
 * ¿Qué es este constructor? Es el constructor por defecto de la clase `Vacante`. Es un método especial que se
 * llama automáticamente cuando creas un nuevo objeto `Vacante` sin proporcionar ningún argumento. ¿Cómo
 * funciona? Cuando escribes `new Vacante()`, este constructor se ejecuta. En este caso, no realiza ninguna
 * inicialización explícita de los atributos, pero Java se encarga de asignar valores por defecto (0 para
 * números, `null` para objetos, `false` para booleanos). Nota: El atributo `imagen` ya tiene un valor por
 * defecto asignado en su declaración. ¿Por qué se implementa así? - **Creación de Objetos**: Permite crear
 * instancias de la clase `Vacante`. - **Flexibilidad**: Es útil cuando quieres crear un objeto vacío y luego
 * establecer sus propiedades usando los métodos `setter`. - **Requisito de Frameworks**: Algunos frameworks
 * (como Spring o JPA) requieren un constructor sin argumentos para poder crear objetos automáticamente.
 */
public Vacante() {
...
/**
 * ¿Qué es esta enumeración (`enum`)? `NivelSalarial` es un tipo especial de clase en Java que representa un
 * conjunto fijo de constantes. En este caso, define los posibles niveles de salario de una vacante: BAJO,
 * NORMAL y ALTO. ¿Cómo funciona? Cada constante de la enumeración (`BAJO`, `NORMAL`, `ALTO`) es un objeto de
 * tipo `NivelSalarial`. Estas constantes pueden tener atributos y métodos propios, como `textoDescriptivo` y
 * `getTextoDescriptivo()`. El constructor `NivelSalarial(String texto)` se usa para inicializar el
 * `textoDescriptivo` de cada constante cuando se define. ¿Por qué se implementa así? - **Seguridad de Tipo**:
 * Restringe los valores posibles a un conjunto predefinido, evitando errores de escritura o valores inválidos
 * (ej. no puedes tener un `NivelSalarial` que no sea BAJO, NORMAL o ALTO). - **Legibilidad**: Hace que el
 * código sea más claro y fácil de entender al usar nombres descriptivos en lugar de números o cadenas
 * arbitrarias. - **Comportamiento Asociado**: Permite asociar datos (como el `textoDescriptivo`) y
 * comportamiento (métodos) directamente a cada constante de la enumeración.
 */
...
/**
 * ¿Qué hace este método? `@Override` indica que este método está sobrescribiendo un método de la clase padre
 * (`Object`). El método `toString()` devuelve una representación en formato de cadena de texto del objeto
 * `Vacante`. ¿Cómo lo logra? Construye una cadena de texto que incluye los nombres y valores de todos los
 * atributos importantes de la vacante (id, nombre, descripcion, fecha, salario, destacado, imagen). Esto se
 * hace concatenando cadenas y los valores de los atributos. ¿Por qué se implementa así? - **Depuración**: Es
 * extremadamente útil para depurar tu aplicación. Cuando imprimes un objeto `Vacante` (ej.
 * `System.out.println(vacante);`), Java llama automáticamente a este método `toString()`, lo que te permite ver
 * el estado completo del objeto de forma legible en la consola. - **Logging**: Facilita el registro de
 * información sobre los objetos en los logs de la aplicación. - **Representación Amigable**: Proporciona una
 * forma rápida y sencilla de obtener una descripción textual de un objeto.
 */
@Override
public String toString() {
	return "Vacante{" + "descripcion='" + descripcion + '\'' + ", destacado=" + destacado + ", detalles='"
			+ detalles + '\'' + ", estatus='" + estatus + '\'' + ", fecha=" + fecha + ", id=" + id
			+ ", imagen='" + imagen + '\'' + ", nombre='" + nombre + '\'' + ", salario=" + salario + '}';
}
}
